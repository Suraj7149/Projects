...
# #Button(root, text = "Exit", font = Font(family="Ubuntu", size = 18, weight='bold'), 
# #        bg="black", fg = "white", command=root.destroy).pack(fill=X,side=BOTTOM)
# # removing the title bar in windows
...
# title_bar = Frame(root, bg =RGRAY, relief="raised", bd=1)
# title_bar.pack(expand=1, fill=X, anchor=N)

# # bind the title bar.
# title_bar.bind("<B1-Motion>", move_app)

# title_label = Label(title_bar, text="Fake Title Bar", font = Font(family="Ubuntu", size = 11, weight='bold'), bg = "black", fg = "white")
# title_label.pack(side=LEFT, pady=1)

# Button(title_bar,width=3, text = "X", bg="black", font = Font(family="Ubuntu", size = 10, weight='bold'), fg = "white", command=root.destroy).pack(side=RIGHT)
...
root.title("Fake Title Bar") 
root.overrideredirect(True) # turns off title bar, geometry
root.geometry('800x600+75+75') # set new geometry the + 75 + 75 is where it starts on the screen
